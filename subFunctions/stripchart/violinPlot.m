function violins = violinPlot(data, cats, markerColors, varargin)
...
% This code is released under the terms of the BSD 3-clause license
    ...
    elseif hascategories && numel(data) == numel(cats)
        ...
        colormapSize = length(catnames);        
        c1 = markerColors.c1;
        c2 = markerColors.c2;
        
        colors_p = [linspace(c1(1),c2(1),colormapSize)', linspace(c1(2),c2(2),colormapSize)', linspace(c1(3),c2(3),colormapSize)'];
        
        for n=1:length(catnames)
            thisCat = catnames{n};
            thisData = data(cats == thisCat);
            violins(n) = Violin2(thisData, n, 'ViolinColor', colors_p(n,:), varargin{:});
%             violins(n) = Violin2(thisData, n, varargin{:});  
        end
        set(gca, 'xtick', 1:length(catnames), 'xticklabels', catnames);
